...
void AValhallaSiegeWeapon::SetPrimarySeat(AActor* InActor)
{
	PrimarySeat = InActor;
}

void AValhallaSiegeWeapon::SetSecondarySeat(AActor* InActor)
{
	SecondarySeat = InActor;
}

FVector AValhallaSiegeWeapon::VehicleSecondarySeatGetOffLocation()
{
	return SecondarySeatGetOffLocation->GetComponentLocation();
}
	...
	if (AValhallaPlayerController* NewPlayerController = Cast<AValhallaPlayerController>(GetController()))
	{
		if (HasAuthority())
		{
			if (ISeatInterface* SeatInterface = Cast<ISeatInterface>(NewPlayerController->ControlledPawn))
			{
				...
				SeatInterface->SeatInitialize();
				SeatInterface->VehicleInitialize();
			}
